...
function preload(){
    ...
    tamFuente = (windowWidth+windowHeight)/150;
    ...
    tamCanvasSonidoX = int(370*16);//aparentemente hay un limite de tamaños de los canvas?
    tamCanvasSonidoY = int(370*8);// antes estas dos variables eran el doble y solo cargaba canvasSpectrums[0]
    ...
    canvasSpectrums[0] = createGraphics(tamCanvasAncho, tamCanvasSonidoY);
    canvasSpectrums[1] = createGraphics(tamCanvasSonidoX, tamCanvasAncho);
    canvasSpectrums[2] = createGraphics(tamCanvasAncho, tamCanvasSonidoY);
    canvasSpectrums[3] = createGraphics(tamCanvasSonidoX, tamCanvasAncho);
...
function draw(){
    ...
    trackMouse();
    console.log("Velocidad: "+slider1+" / Tamaño: "+slider2+" / vel: "+vel+" / velScroll: "+velScroll+" / tamF: "+tamFuenteMult);
    monitoreoParametros();
}

function monitoreoParametros(){
    textSize(tamFuente*tamFuenteMult);
    stroke(0);
    fill(255,0,255,alhphaGUI);
    text("vel: "+slider1, xSlider, ySlider);
    fill(0,255,0,alhphaGUI);
    text("tam: "+slider2, xSlider, ySlider+(tamFuente*tamFuenteMult));
}
