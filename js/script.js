...
function playPause(){
    for(let i = 0 ;i < track.length; i++){
        if (track[i].sonido.isPlaying()) {track[i].sonido.pause();selectCanvas[6].state = false;} 
        else {track[i].sonido.loop();selectCanvas[6].state = true;}
  }
}
...
function guardar(){selectCanvas[4].state = true;}

function masVelo(){slider1+=0.01;}
function menosVelo(){slider1-=0.01;}
function masTam(){slider2+=0.5;}
function menosTam(){slider2-=0.5;}
